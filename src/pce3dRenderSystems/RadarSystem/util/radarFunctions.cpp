...
namespace {
constexpr const char* log_name = "radar_fns";
}
...
const double RAD_PER_PIXEL = 0.00087; //!< 0.5 degrees
} /* namespace */
...
std::optional<std::pair<int, int>> viewSpherePointToPixel(const glm::dvec3& point)
{
    ...
    double z = point.z;
    //LOG_DEBUG(log_name) << "point: " << int(point.x) << ", " << int(point.y)<< ", " << int(point.z);
    if (point.z <= -2 /* experiment with this value to get it right */)
    {
        //LOG_DEBUG(log_name) << "entity is behind camera.";
        return std::nullopt;
    }
    if (point.z == 0)
    {
    ...
    const double xz_rad = atan( point.x / z );
    const double yz_rad = atan( point.y / z );
    const double eighty_deg_rad = 1.39;
    if (abs(xz_rad) > eighty_deg_rad || abs(yz_rad) > eighty_deg_rad)
    {
        //LOG_DEBUG(log_name) << "entity is behind field of view boundary.";
        return std::nullopt;
    }

    //LOG_DEBUG(log_name) << "yz_rad, xz_rad: " << int(yz_rad) << ", " << int(xz_rad);
